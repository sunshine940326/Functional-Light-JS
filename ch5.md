...
然而，并不是所有的纯函数都是数学概念上的幂等，因为它们不需要返回一个值适合作为输入来返回的值。思考一下：
...
我们需要对 `rememberNumbers(..)` 有的一个更粗鲁的定义来防止 `fn(..)` 改变它的闭合的 `nums` 变量的引用。
...
您经常会发现这些副作用，没被观察到的用于优化性能的操作。例如：
...
这种技术的有效性有限，主要是因为如果你不能将函数本身改为纯的，你也不可能修改其周围的代码。然而，如果可能，探索它是有帮助的，因为它是我们修复中最简单的。
...
这个技术的成功将取决于你所做的**复制**的彻底性。使用 `userList.slice()` 在这里不起作用，因为这只会创建一个 `userList` 数组本身的浅拷贝。数组的每个元素都是一个需要复制的对象，所以我们需要格外小心。当然，如果这些对象在它们之内有对象（可能会这样），则复制需要更加完善。
...
纯函数是最好避免副作用。纯函数给定相同输入时总返回相同输出，并且没有副作用。引用透明更近一步的状态是 —— 更多的是一种脑力运动而不是文字行为 —— 纯函数的调用是可以用它的输出来代替，并且程序的行为不会被改变。
